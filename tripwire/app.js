...
var code="degisendosya=yeni8.txtozeti=7c73b15937f391635d5ac6a245bf0dcc";
var arr=[];
var arr2=[];
var arr3=[];
var arr4=[];
...
$('input.btn5').live('click', function() {
	...
	var request = sdcard2.addNamed(hash, "kritikk.txt");
	request.onsuccess = function () {  
		alert("yazılıyorr");
   name = $(this).result;
...
$('input.btn10').live('click', function() {
  ...
  neww=a.join("\n");
	/*for(j=0;j<arr2.length;j++){
		if(arr2[j]!=arr3[i]){
			alert("normal dosyalarda değişilik var "+" "+arr2[j]);
			//$("#rapor").html("normal dosyalarda değişiklik var bunlar :"+" "+arr2[j])
		 b[i]=arr2[i];
		}
	}
...
var encodedData = window.btoa(send);
function sendMail(){
